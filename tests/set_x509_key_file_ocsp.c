...
static const gnutls_datum_t ocsp_resp1 =
...
static const gnutls_datum_t ocsp_resp2 =
...
static void run_with_2responses(void)
{
	int ret;
	...
	if (fp == NULL)
		fail("error in fopen\n");
	assert(fwrite(server_ca3_localhost_cert, 1, strlen((char*)server_ca3_localhost_cert), fp)>0);
	...
	if (fp == NULL)
		fail("error in fopen\n");
	assert(fwrite(server_ca3_localhost6_cert, 1, strlen((char*)server_ca3_localhost6_cert), fp)>0);
	assert(fwrite(server_ca3_key_pem, 1, strlen((char*)server_ca3_key_pem), fp)>0);
	fclose(fp);
	...
	test_cli_serv(xcred, "NORMAL", &ca_cert, "localhost", &ocsp_resp1, check_response, NULL); /* the DNS name of the first cert */

	test_cli_serv(xcred, "NORMAL", &ca3_cert, "localhost6", &ocsp_resp2, check_response, NULL); /* the DNS name of the first cert */
